...
public class BotCore {
    ...
    @Getter
    protected TextLog callbacks;

    // Members
    @Getter
    protected ChatMembersCache members = new ChatMembersCache();

    // Info2
    @Getter
    protected Info2Resource info2 = new Info2Resource(getConfig().getDataDirName(), INFO2, config.getEncoding());

    // RDBs
    @Getter
    protected HashMap<String, RDBResource> rdbStore = new HashMap<>();

    // DBs
    @Getter
    protected MyObjectMapper mapper;
    ...
    protected BotConfig checkConfig(BotConfig config) {
        if (config != null) {
    ...
    protected boolean wasAnsweredRecently(Update update) {
        ...
        return wasAnsweredRecently(id);
    }

    protected void addToAnsweredCache(Update update) {
        Integer id = idOf(update);
        addToAnsweredCache(id);
    }

    protected boolean wasAnsweredRecently(Integer id) {
    ...
    protected void addToAnsweredCache(Integer id) {
        if (msgCache != null && id != null) {
